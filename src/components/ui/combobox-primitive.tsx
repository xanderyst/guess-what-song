...
>(({ onKeyDown, onMouseDown, onFocus, onBlur, ...props }, ref) => {
  ...
  const [tempInputValue, setTempInputValue] = React.useState(inputValue); // Temporary input value
  ...
  return (
    <CommandPrimitive.Input
      ...
      disabled={disabled}
      required={required}
      value={isComposing ? tempInputValue : inputValue} // Show phonetics while composing
      onCompositionStart={() => setIsComposing(true)}
      onCompositionUpdate={(e) => setTempInputValue(e.currentTarget.value)} // Update input while composing
      onCompositionEnd={(e) => {
        setIsComposing(false);
        onInputValueChange(e.currentTarget.value, "inputChange");
      }}
      onValueChange={(search) => {
        ...
        if (isComposing) {
          setTempInputValue(search); // Update temp value while typing
        } else {
